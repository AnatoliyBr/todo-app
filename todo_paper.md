# Мой опыт создания REST API сервера для введения todo-списков
...
* [Интерфейсы хранилища (Store) и репозиториев](#интерфейсы-хранилища-store-и-репозиториев)
* [Реализации интерфейсов хранилища (Store) и репозиториев](#реализации-интерфейсов-хранилища-store-и-репозиториев)
...
2. Метод `FindByID`
    ...
    * Чтобы **стандартизировать ошибки**, которые возвращают разные реализации хранилища (`sqlstore` и `teststore`), создал кастомную ошибку `ErrRecordNotFound`. А именно в директории `internal/store` создал файл `error.go`, в котором определил новую ошибку с помощью функции `errors.New`.
...
В частности, в файле `usecase.go` создал структуру `AppUseCase` с приватным полем `store` типа интерфейс хранилища, конструктором `NewUseCase`, который принимает интерфейс хранилища и возвращает указатель на `AppUseCase` и соответствующими методами.
